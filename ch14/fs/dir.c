...
/* 
    打开根目录 
    目录内的偏移设置为0
    本质是调用inode_open 将对应的inode的指针赋给dir结构的indode 指针
*/
...
/* 在分区part上打开i结点为inode_no的目录并返回目录指针(即dir结构体指针) 
    本质是调用inode_open 将对应的inode的指针赋给dir结构的indode 指针
*/
...
/* 在part分区内的pdir目录内寻找名为name的文件或目录,（注意同时搜索，文件或者目录！！）
 * 找到后返回true,否则返回false 
    dir_e是传出参数
    ----------------------------------------------------------------------------------
    先从pdir中获取inode指针，inode结构体有一个长度为13且存放对应扇区地址的数组。
    按照这几个指针从硬盘上读入相应的数据，再从这数据中一一遍历寻找对应的dir_entry结构
    找到就在dir_e中填充这个dir_entry结构
 */
bool search_dir_entry(struct partition* part, struct dir* pdir, \
   ...
   /* 12个直接块大小+128个间接块,inode中用uint32（4字节）记录一个扇区地址，共560字节 */
   ...
   /* 块地址为0时表示该块中无数据,继续在其它块中找 */
	 ...
	 /* 若找到了,就直接复制整个目录项 */
        if (!strcmp(p_de->filename, name)) {
            //找到了对应的目录及或者文件，将p_de后的dir_entry_size个字节的内容拷贝至传出参数中
...
/* 将目录项p_de写入父目录parent_dir中，io_buf由主调函数提供 
    ----------------
    在140个扇区块中循环搜索，如果扇区未创建，那么就创建它，分3种情况
       如果找到一个扇区，再在这个扇区种循环搜索一个空闲的区域，空闲区域代大小为dir_entry的大小
*/
bool sync_dir_entry(struct dir* parent_dir, struct dir_entry* p_de, void* io_buf) {
   ...
   while (block_idx < 140) {  // 文件(包括目录)最大支持12个直接块+128个间接块＝140个块
      block_bitmap_idx = -1;
      if (all_blocks[block_idx] == 0) {   // 如果没有分配块，那么分三种情况分配块
        ...
        /* 块已经分配了，直接将新目录项p_de写入新分配的间接块，然后返回即可 */
        memset(io_buf, 0, 512);
   ...
   //遍历了目录对应的所有140块，没有找到空闲空闲，那么返回false
   printk("directory is full!\n");
   return false;
}

/* 把分区part目录pdir中编号为inode_no的目录项删除 , io_buf 是与硬盘交互的缓冲区，由主调函数提供*/
bool delete_dir_entry(struct partition* part, struct dir* pdir, uint32_t inode_no, void* io_buf) {
   ...
   block_idx = 0;
   while (block_idx < 140) {
      //表示当前目录中最初的那个块，即“.”目录项，如果这个目录中处了"."".."两个目录项以外没有其他目录项的话，就将这个块收回
   ...
   /* 更新i结点信息并同步到硬盘 */
      ...
      dir_inode->i_size -= dir_entry_size;//减少i_size
...
/* 读取目录,成功返回1个目录项,失败返回NULL 
   连续对相同的目录调用而不使用sys_rewinddir将pos重置，那么将接着返回目录中的下一个目录项
*/
struct dir_entry* dir_read(struct dir* dir) {
   ...
   while (block_idx < block_cnt) {
      if (dir->dir_pos >= dir_inode->i_size) {
         //如果dir_pos大于等于文件尺寸，那么说明已经遍历了所有的目录项，直接放回NULL，dir_pos在执行 sys_opendir时已经置为0
         return NULL;
      }
...
int32_t dir_remove(struct dir* parent_dir, struct dir* child_dir) {
   ...
   /* 在父目录parent_dir中删除子目录child_dir对应的目录项 （硬盘）*/
   delete_dir_entry(cur_part, parent_dir, child_dir_inode->i_no, io_buf);

   /* 回收inode中i_secotrs中所占用的扇区,并同步inode_bitmap和block_bitmap  （内存）*/
