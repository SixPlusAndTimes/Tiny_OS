...
static void wash_path(char* old_abs_path, char* new_abs_path) {
   assert(old_abs_path[0] == '/');//原来的路径也是一个绝对路径
...
/* 将path处理成不含..和.的绝对路径,存储在final_path 
    现检查path是否是绝对路径，如果不是就调用getcwd()系统调用获取当前绝对路径，再拼接上path后组成绝对路径
    最后将拼接成的绝对路径传入wash_path函数中即可
*/
...
/* ls命令的内建函数 
   选项 ：-l  显示详细信息，如果没有这个选项，则只是显示文件名而不显示文件属性
         -h  帮助信息 
*/
void buildin_ls(uint32_t argc, char** argv) {
   ...
   if (pathname == NULL) {	 // 若只输入了ls 或 ls -l,没有输入操作路径,默认以当前路径的绝对路径为参数.
      ...
      make_clear_abs_path(pathname, final_path);//将路径中的.. 和 .处理掉
      pathname = final_path;
   }

   if (stat(pathname, &file_stat) == -1) {//调用stat查看file_stat,如果没有这个文件则会返回-1
