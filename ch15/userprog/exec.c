...
/* 
    elf文件简单回顾 ：
    1. 在链接器眼中，elf由各种节组成，连接器按照节的属性将它门合并成几个段
    2. 在加载器眼中，elf由各种段组成，如果段的属性是LOAD那么他将按照编译器给定的虚拟地址将段加载到内存，再按elf头给定的程序入口地址跳转执行
...
struct Elf32_Ehdr {
   ...
   Elf32_Off     e_phoff; //程序头表在elf文件中的品偏移
   Elf32_Off     e_shoff;
   Elf32_Word    e_flags;
   Elf32_Half    e_ehsize;
   Elf32_Half    e_phentsize;//程序头表中每个条目的字节大小
   Elf32_Half    e_phnum;//段的数量
...
struct Elf32_Phdr {
   ...
   Elf32_Addr p_vaddr;       //段在内存的起始虚拟地址
   Elf32_Addr p_paddr;//忽略
...
int32_t sys_execv(const char* path, const char* argv[]) {
   ...
   return 0;//根本不会返回，jmp intr_exit之后就开启了新的执行流
}
