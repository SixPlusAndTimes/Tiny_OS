...
static void* vaddr_get( enum pool_flags pf, uint32_t pg_cnt) {
    ...
    uint32_t cnt = 0;
    if(pf == PF_KERNEL) {
        //如果内存池是内核的。那就使用bitmap_scan来处理 kernel_vaddr.vaddr_bitmap 这个 全局变量
        //bitmap_scan:在位图中申请连续cnt个位,成功则返回第一个位的小标，失败返回-1 */
        bit_idx_start = bitmap_scan(&kernel_vaddr.vaddr_bitmap, pg_cnt);

        if(bit_idx_start == -1) {
            //kernel_vaddr.vaddr_bitmap中已经没有空位了，返回null
            return NULL;
        }
...
static void page_table_add(void* _vaddr, void* _page_phyaddr) {
    ...
    // 先判断pde中的内容的第0位是否为1，为1则说明对应的页表已经存在。如果不存在，那么就该先创建页表然后再使用 *pte，否则会引发pagefault异常
    // 当前已经创建了一个页表，可以支持1024个pte，一个页表可以支持4MB内存，所以在4MB范围内增加pte时，就不需要额外申请物理页来创建页表。
    if(*pde & 0x00000001) {
        ASSERT(!(*pte & 0x00000001));//按理来说，在映射地址的时候，pte不会存在
    ...
    } else { //页目录项不存在,表示对应的页表还没有建立
        PANIC("pte repeat");
        // 页表使用的物理内存一律从内核空间中分配，因为不能让用户进程直接接触页表
        uint32_t pde_pyhaddr = (uint32_t)palloc(&kernel_pool);

        *pde = (pde_pyhaddr | PG_US_U | PG_RW_W | PG_P_1);

        // 将新分配的物理页清零
        // 但是在开启分页后，一切的访问都是基于虚拟地址，pte变量存储的是pte页表项的虚拟地址
        // 用pte（是pte的虚拟地址）取高20位即得到pte所在页表的起始虚拟地址，将得到的虚拟地址传给memset函数，让它初始化PG_SIZE个字节，于是就将整个物理页表清零了
...
void* get_kernel_pages(uint32_t pg_cnt) {
    lock_acquire(&kernel_pool.lock);//第十一章新增锁操作

    void* vaddr = malloc_page(PF_KERNEL,pg_cnt);
    // put_str("\ngot kernel_pages_virtual_start ");
    // put_int((uint32_t)vaddr);
    if(vaddr != NULL) {
        memset(vaddr,0,pg_cnt*PG_SIZE);//将分配得到的页做初始化处理
    }
    lock_release(&kernel_pool.lock);//第十一章新增锁操作
    return vaddr;
}
...
uint32_t addr_v2p(uint32_t vaddr) {
    uint32_t* pte = pte_ptr(vaddr);//先取得pte项的虚拟地址
    //对pte的虚拟地址解引用即可得到其中存放的值，pte值的前20位即可定位物理的地址中的页框位置，再加上虚拟地址的后12位，就是完整的物理地址
    return ((*pte & 0xfffff000) + (vaddr & 0x00000fff));
}

//初始化内存池
// all_mem是所有可用的真实内存大小，在LOADER中计算得到具体的内存大小，且被存放在了物理地址0xb00处
